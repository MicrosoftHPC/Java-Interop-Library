 ...
 * not try to create or attach to a session. It should report the error to their
 * helpdesk. There is something wrong with app's implementation or installation
 */
public class MessagePreemptionOutInterceptor extends AbstractSoapInterceptor {
    ...
    public void handleMessage(SoapMessage message) throws Fault {
        ServiceContext.Logger.traceEvent(Level.INFO, "MessagePreemptionOutInterceptor : handle outbound message");

        if (hpcHostWrapper.enableMessageLevelPreemption) {
            String id = (String) message.getExchange().get("ID");
            ServiceContext.Logger.traceEvent(Level.INFO, "MessagePreemptionOutInterceptor : id=" + id);
            ...
            if (!this.hpcHostWrapper.processingMessageIds.contains(id)) {
                ...
                if (this.hpcHostWrapper.receivedCancelEvent) {
                    ServiceContext.Logger.traceEvent(Level.INFO, "MessagePreemptionOutInterceptor : receivedCancelEvent");
